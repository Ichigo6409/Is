...
class GoodsReceiptController extends Controller
{
    ...
    public function index(Request $request): Response
    {
        ...
        $receiptsData = collect($receipts->items())->map(function ($r) use ($ordersMap) {
            $order = $ordersMap->get((string) $r->purchase_order_id);

            // Formato seguro de received_at
            $receivedAt = null;
            if ($r->received_at) {
                try {
                    if (is_string($r->received_at)) {
                        $receivedAt = $r->received_at;
                    } else {
                        $receivedAt = \Illuminate\Support\Carbon::parse($r->received_at)->toIso8601String();
                    }
                } catch (\Throwable $e) {
                    $receivedAt = null;
                }
            ...
            return [
                ...
                'received_by' => (string) ($r->received_by ?? 'SYSTEM'),
    ...
    public function store(Request $request, InventoryService $inventoryService): RedirectResponse
    {
        ...
        if (!$order) {
            return redirect()->route('equipo4.recepciones.index')->withErrors(['error' => 'Orden de compra no encontrada.']);
        }

        if ($order->status !== 'AUTORIZADA') {
            return redirect()->route('equipo4.recepciones.index')->withErrors([
                'error' => 'Solo se pueden recibir OCs en estado AUTORIZADA. Estado actual: ' . $order->status
            ]);
        }
        ...
        try {
            ...
            foreach ($stockMovementsDone as $sm) {
                try {
                    ...
                    Log::critical('Fallo compensacion stock.', ['error' => $ex->getMessage()]);
                }
            }

            if (!empty($createdReceiptItemIds)) {
                try {
                    GoodsReceiptItem::whereIn('_id', $createdReceiptItemIds)->delete();
                } catch (\Throwable $ex) {}
            }

            if ($receipt !== null && $receipt->_id) {
                try { $receipt->delete(); } catch (\Throwable $ex) {}
            }

            return redirect()->route('equipo4.recepciones.index')->withErrors([
                'error' => 'No se pudo registrar la recepcion: ' . $e->getMessage()
            ]);
        }

        return redirect()->route('equipo4.recepciones.index')->with('success', 'Recepcion ' . $folio . ' registrada.');
    }
    ...
    public function destroy(string $id): RedirectResponse
    {
        ...
        if (!$receipt) abort(404, 'Recepcion no encontrada.');
