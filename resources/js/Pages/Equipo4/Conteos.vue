...
const openCreateModal = () => {
  ...
  if (props.warehousesList.length > 0) {
    createForm.warehouse_id = props.warehousesList[0]._id
  }
...
const openCaptureModal = async (row) => {
  ...
  try {
    const response = await fetch(`/equipo4/conteos/${row._id}/details`, {
      headers: { 'Accept': 'application/json' }
    })
    const data = await response.json()
    captureItems.value = data.items.map(i => ({
      ...i,
      counted_qty_input: i.counted_qty || 0
    }))
  } catch (e) {
    console.error(e)
  } finally {
...
const saveCapture = () => {
  ...
  const items = captureItems.value.map(i => ({
    item_id: i._id,
    counted_qty: Number(i.counted_qty_input) || 0
  }))
...
const closeCount = () => {
  ...
  // Contar alertas sospechosas antes de cerrar
  const alerts = captureItems.value.filter(i => hasAlert(i) !== null).length

  let msg = `¿Cerrar el conteo ${selectedCount.value.folio} y aplicar los ajustes al inventario? Esta acción no se puede deshacer.`
  if (alerts > 0) {
    msg = `⚠️ Hay ${alerts} diferencia(s) sospechosa(s) en este conteo. ¿Seguro que quieres cerrar y aplicar los ajustes? Esta acción no se puede deshacer.`
  }
...
const calculatedDifference = (item) => {
  return (Number(item.counted_qty_input) || 0) - item.expected_qty
}

/**
 * Devuelve el nivel de alerta si la diferencia es sospechosa.
 * - 'critical': diferencia > 500 O counted > 10x expected
 * - 'warning': counted > stock_max registrado
 * - null: sin alerta
 */
const hasAlert = (item) => {
  ...
  // Critical: diferencia absoluta muy grande o multiplicador exagerado
  if (Math.abs(diff) > 500) return 'critical'
  if (expected > 0 && counted > expected * 10) return 'critical'

  // Warning: excede el stock maximo registrado
      ...
      <div class="bg-white rounded-2xl shadow-xl border border-slate-200 w-full max-w-md p-6 space-y-4">
        ...
        <form @submit.prevent="submitCreate" class="space-y-4">
          <div>
            ...
            <p v-if="createForm.errors.warehouse_id" class="mt-1 text-xs text-rose-600">{{ createForm.errors.warehouse_id }}</p>
          </div>

          <div>
            <label class="block text-xs font-semibold uppercase text-slate-600 mb-1">Notas</label>
            <textarea v-model="createForm.notes" rows="2" placeholder="Observaciones del conteo" class="w-full px-3 py-2 text-sm rounded-lg border border-slate-300 focus:border-[#0284C7]"></textarea>
          </div>

          <p class="text-xs text-slate-500 bg-slate-50 rounded-lg p-3">
            Al crear el conteo, se precargarán todos los productos con existencia en el almacén seleccionado.
          </p>
      ...
      <div class="bg-white rounded-2xl shadow-xl border border-slate-200 w-full max-w-4xl p-6 space-y-4 my-8">
        ...
        <div v-else class="space-y-4">
              ...
              <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-slate-100">
                  <tr v-for="item in captureItems" :key="item._id" :class="alertClass(item)">
                    ...
                    <td class="px-3 py-2">
                      <input v-model.number="item.counted_qty_input" type="number" min="0" class="w-full px-2 py-1 text-xs rounded border border-slate-300 focus:border-[#0284C7] text-right" />
                    </td>
                    <td class="px-3 py-2 text-right font-semibold"
                      :class="calculatedDifference(item) === 0 ? 'text-emerald-600' : 'text-rose-600'">
                  ...
                  <tr v-if="captureItems.length === 0">
                    <td colspan="7" class="px-3 py-4 text-center text-xs text-slate-500">Sin items en este conteo.</td>
                  </tr>
                </tbody>
              </table>
            </div>
          ...
          <div class="bg-amber-50 border border-amber-200 rounded-lg p-3 text-xs text-amber-800">
            <strong>Nota:</strong> Las filas en amarillo/rojo indican diferencias sospechosas (exceden el stock máximo o son muy grandes). Puedes guardarlas igual — el sistema no bloquea — pero revisa que no sea un error de captura.
          </div>
          ...
          <div class="flex justify-end gap-2 pt-3 border-t border-slate-100">
            ...
            <button type="button" @click="saveCapture" class="px-4 py-2 text-sm font-semibold rounded-lg border border-[#0284C7] text-[#0284C7] bg-white hover:bg-slate-50 transition">
              Guardar captura
            </button>
            <button type="button" @click="closeCount" class="px-4 py-2 text-sm font-semibold rounded-lg bg-[#00338D] text-white hover:bg-[#0284C7] transition">
    ...
    <div v-if="showDeleteModal" class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4">
      <div class="bg-white rounded-2xl shadow-xl border border-slate-200 w-full max-w-md p-6 space-y-4">
        ...
        <p class="text-sm text-slate-600">
          Vas a eliminar el conteo <strong>{{ selectedCount?.folio }}</strong>. Solo se permite si está en Borrador.
        </p>
